...
namespace Value_to_Text_Converter
{
    class Value_To_Text_Methodes
    {
        ...
        public bool Value_Validation(string number, out int err_id, out string err_message)
        {
            ...
            if (number.Length != 0)
            {
                if (number.Length <= 20)//check whether length of the string doesn't exceed max length or if string is empty.
                {
                            ...
                            if (dots_no != 0)//check if the number has fractional part.
                            {
                                if (number.Split('.')[1].Length <= 4)//check if the fractional part length doesn't exceed max limit.
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        err_message = "integral part is out of length range(12 digit)";// set error message describe the error type.
                                        return false;
                                    }
                                }
                                else
                                {
                                    ...
                                    err_message = "fractional part is out of length range(4 digits)";// set error message describe the error type.
                                    return false;
                                }
                            }
                            else
            ...
            else
            {
                ...
                err_message = "Empty Field";// set error message describe the error type.
                return false;
            }
        }
        /// <summary>
        ...
        /// <summary>
        /// Convert a number to text in two langauages arabic or english & add name of currency in case currency is added.
        /// </summary>
        /// <param name="number">the number that will be converted</param>
        /// <param name="lang">the indicator of the language that text will be in it's format</param>
        /// <param name="cur">the currency that will be added</param>
        /// <returns>string text that expresses the number but in words according to inputted language with addition of currency incase a currency is added</returns>
        public string Numeric_To_Text(string number , string lang ,string cur)
        {
            int err_id;
            string err_message;
            if (Validation(number, lang, out err_id, out err_message))
            {
                ...
                {
                    if (cur != null && cur != "")//check whether there's no currency added.
                        return "Only " + En_Converter(number) + " " + cur + ".";//returns the text in english equivalent to the number with word only & the currency that is added.
                    else
                        return En_Converter(number) + ".";//returns the text in arabic equivalent to the number without any addition.
                }
                else
                {
                    if (cur != null && cur != "")//check whether there's no currency added.
                        return "." + "فقط" + " " + Ar_Converter(number) + " " + cur + " لا غير";//returns the text in english equivalent to the number with word only & the currency that is added.
                    else
                        return "." + Ar_Converter(number);//returns the text in arabic equivalent to the number without any addition.
                }
            }
            else
            {
